...
begin
  ...
  where identity_type = 'phone_e164' and identity_value = v_phone
  limit 1;

  if v_click_id is not null then
    select lead_id into v_click_lead
    from public.lead_events
    where event_type = 'whatsapp_click' and event_key = v_click_id
    limit 1;
  else
    select le.lead_id, le.event_key into v_click_lead, v_match_click_id
